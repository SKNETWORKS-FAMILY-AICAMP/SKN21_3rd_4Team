...
- **팀/프로젝트**
  ...
  - [프로젝트 주제 및 주제 설명 및 선정 이유](#프로젝트-주제-및-주제-설명-및-선정-이유)
- **데모/기능**
  - [Application의 주요 기능](#application의-주요-기능)
  - [프로젝트 전체 흐름도](#프로젝트-전체-흐름도)
- **구성/스택**
  - [프로젝트를 구성하는 디렉토리들과 파일들의 구조](#프로젝트를-구성하는-디렉토리들과-파일들의-구조)
  - [Tech Stack](#tech-stack)
- **데이터/인제스션**
  - [수집 데이터 설명](#수집-데이터-설명)
  - [강의 데이터(lecture) 전처리 및 임베딩](#강의-데이터lecture-전처리-및-임베딩)
  - [Python 공식 문서 (RST)](#2%EF%B8%8F%E2%83%A3-python-%EA%B3%B5%EC%8B%9D-%EB%AC%B8%EC%84%9C-rst)
- **검색/평가**
  - [Retrieval 실험 결과 (평균 유사도)](#retrieval-실험-결과-평균-유사도)
  - [하이브리드 검색 (Vector + Keyword + BM25)](#하이브리드-검색-vector--keyword--bm25)
  - [RAG 평가 결과](#rag-평가-결과)
- **회고**
  - [회고](#회고)
...
<p align="center">
  <figure>
    ...
    <figcaption>프로젝트 전체 흐름도</figcaption>
  </figure>
  <figure>
    <img src="image/langgraph.png" alt="langgraph" width="370"/>
    <figcaption>LangGraph Workflow</figcaption>
  </figure>
</p>
...
2차 : 프롬프트 개선으로 평균 점수 개선

Answer relevancy 점수를 0.8352로 올렸으나 Context Recall이 1.0에서 0.8583으로 떨어짐

- 프롬프트 개선 내용 : 기존 프롬프트는 context에대한 강제성이 너무 강했음. 유연하게 답변할 수 있도록 프롬프트 개선
- 데이터셋 난이도 조절 : 데이터셋이 너무 쉬운 경우가 많음. 데이터셋 난이도를 조금 높임
...
| 이름 | 소감 |
|-----------|-----------|
| **김가람** | PM(팀장)과 프로젝트 설계, 분석 Agent 및 LangGraph 구조화 작업을 맡아서 진행했습니다. 짧은 일정이라 병렬적으로 담당파트를 진행하기 위해 역할 분담과 설계에 신경을 많이 썼습니다. 분석 Agent를 빠르게 끝내고 Retrieval 검색 결과를 올릴 수 있는 방법을 같이 고민해봤고 Tavily Search Tool을 추가해서 문서 유사도가 낮은 검색 결과들을 보완했습니다. RAGAS 평가를 진행 과정에서 분석 프롬프트를 개선할 수 있었고, 강사님께서 자연어의 경우에는 정성평가의 결과가 더 중요하다고 말씀해주셔서 정성평가에 집중했습니다. 그 결과 정성평가에서는 높은 만족하는 결과를 가져왔습니다. |
| **신지용** | 강의 노트·Python 공식문서 RST 전처리를 통해 검색 가능한 형태로 정제하고, 검색 라우터/번역 프롬프트와 하이브리드 검색 로직을 구현했습니다. 특히 Vector/Keyword/BM25 가중치 튜닝과 한글→영문 키워드 변환, 단일/복합 쿼리 처리에서 유사도를 안정적으로 끌어올리는 과정이 가장 어려웠습니다. 앞으로는 쿼리 전처리(동의어/복합명사 분해/오탈자), 재현성 있는 스코어링(정규화 기준, 정렬 안정화)과 평가 자동화를 더 고도화해 검색 품질을 지속적으로 개선하고 싶습니다. |
| **최자슈아주원** | 강의노트를 전처리하여 Vector DB에 저장하는 과정에서 데이터 정제 수준이 유사도 개선에 훨씬 더 효과적이라는 사실을 직접 경험하며 학습할 수 있었습니다. 특히 ipynb파일에 마크다운 표현식, 많은 코드 등이 있어 vector db에 바로 저장하는 것은 유사도를 매우 떨어 트리기에 이를 전처리하는 과정에서 많은 시간이 소요되었습니다. 그리고 Front end를 작업하면서 생각보다 쉽지 않다는 것을 알게 되었습니다. 어떠한 기능을 구현할 때 사전에 모든 부분을 생각하고 구성해놓지않으면 수정하는데 많은 시간이 걸리고 다른 문제가 발생할 수 있다는 것을 알게 되었습니다. 그래도 원하는 퀴즈기능을 구현하고 검색창이 비록 오류가 있지만 오류를 억지로 일으키지 않는 이상 정상적으로 작동되는 것에 만족하고 학습에 도움이 될 수 있는 기능을 더 만들고 싶다는 욕심이 생겼습니다. |
| **윤경은** | 이번 프로젝트에서 강의 자료 데이터를 Vector DB로 변환하는 과정을 담당했습니다. 프로젝트 초반 컨디션 이슈로 맡은 역할을 충분히 수행하지 못한 점은 아쉬움으로 남았지만, 이후 좋은 기회로 Vector DB 변환 이후 프롬프트를 직접 수정하며 유사도 변화를 실험해볼 수 있었습니다. 검색(search)과 번역(translate) 프롬프트 설계의 중요성에 주목하여 실험을 진행했으나, 기대와 달리 프롬프트 조정만으로는 유사도 변화가 크지 않다는 점을 확인했습니다. 그 과정에서 프롬프트 튜닝보다 문서 전처리 방식과 데이터 정제 수준이 유사도 개선에 훨씬 더 효과적이라는 사실을 직접 경험하며 학습할 수 있었습니다. 이번에도 역시 좋은 모델과 좋은 프롬프트 이전에 좋은 데이터가 우선되어야 한다는 점을 체감했고, 동시에 ingestion 설계의 중요성을 인지하게 되었습니다. 다음에 기회가 된다면 RAG 시스템에서 ingestion 단계와 데이터 품질 관리를 직접 참여하며 경험해보고 싶습니다. |
| **안혜빈** | 이번 프로젝트를 통해 LLM Reasoning Agent의 역할이 단순한 답변 생성이 아니라,검색 결과를 어떻게 해석하고 어떤 흐름으로 사고하게 할지를 설계하는 일이라는 것을 배웠다. RAG 구조와 Agent 연결 과정에서 발생한 다양한 오류와 시행착오는 쉽지 않았지만,그만큼 LLM 시스템을 전체적으로 이해하는 데 도움이 되었다. 특히 데이터, 프롬프트, 상태(State), Agent 흐름이 서로 맞물려야의미 있는 답변이 나온다는 점을 직접 경험할 수 있었고,이를 통해 LLM을 사용하는 것과 설계하는 것의 차이를 알게 되었다.   |
| **정세연** | 팀원분들 덕분에 즐겁고 편안하게 진행했던 것 같습니다 4차도 화이팅!!  |
